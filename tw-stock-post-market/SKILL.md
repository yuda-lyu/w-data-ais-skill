...
執行前請先驗證套件是否可用：
```bash
node -e "import('axios').then(() => import('wsemi')).then(() => console.log('deps OK'))"
```
...
`generate_report.mjs` 直接讀取盤前調研 Markdown 報告（`tw-stock-research/<YYYYMMDD>/report_<YYYYMMDD>.md`），自動解析其中「個股影響總表」的利多/利空研判個股。

**盤前報告實際結構**：`## 📊 個股影響總表` 之下分 `### ⬆️ 利多` / `### ⬇️ 利空` 兩個區段，每個區段再分三張 `####` 分級子表：

- `#### 🅰️ A 級（法人方向一致 + 題材硬，共 N 檔）`：欄位 `代碼 | 名稱 | 本益比 | 法人動向 | 簡要理由`（最多 5 檔）
- `#### 🅱️ B 級（條件未全，共 N 檔）`：欄位 `代碼 | 名稱 | 信心 | 法人動向 | 簡要理由`（不限檔數）
- `#### ℹ️ C 級（法人方向相反，共 N 檔）`：欄位 `代碼 | 名稱 | 信心 | 法人動向 | 簡要理由`（不限檔數）

> 註：`（共 N 檔）` 檔數標於 `####` 子標題；A 級第 3 欄為「本益比」，B/C 級第 3 欄為「信心」，但三張表第 5 欄皆為「簡要理由」。某分級無個股時該子表會渲染成 `(無)` 而非表格。

**解析規則**：
- 段落標題模糊匹配 `### ⬆️ 利多` / `### ⬇️ 利空`（行內可含尾端檔數），擷取該區段底下 A/B/C 三張子表的**全部**資料列。
- 主要欄位（5 欄）：逐列取 `代碼`(第 1 欄) / `名稱`(第 2 欄) / `簡要理由`(第 5 欄)；A/B/C 三表第 3、4 欄（本益比或信心、法人動向）盤後不使用，僅取理由。
- 舊版扁平表（3 欄，相容）：`代碼 | 名稱 | 簡要理由`，此時 `簡要理由` 取第 3 欄。
- `impact` 從區段標題推導（`⬆️ 利多` / `⬇️ 利空`）。
...
> 盤後技能使用 `axios` 與 `wsemi`（fetch-tw-data-stock / fetch-tw-data-institutional 均為純 Axios 腳本，主控腳本 run_post_market.mjs 另用 wsemi），**不需要** `cheerio` 或 `playwright`（那是盤前技能的依賴）。
