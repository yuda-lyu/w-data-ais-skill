...
| 情境 | 推薦 | 硬規則與樣板 |
|---|---|---|
| 預設；前端生態最廣泛；JSX 慣用者 | **React 18 + Babel inline JSX** | [references/patterns-react.md](references/patterns-react.md) |
| Vue 技術棧客戶；組合式邏輯複用需求高 | **Vue 3 global build + Composition API `setup()`** | [references/patterns-vue3.md](references/patterns-vue3.md) |
| 維護既有 Vue 2 專案；Options API 慣用 | **Vue 2 + Options API** | [references/patterns-vue2.md](references/patterns-vue2.md) |
...
> - [references/patterns-react.md](references/patterns-react.md) —— React 18 + Babel inline JSX
> - [references/patterns-vue3.md](references/patterns-vue3.md) —— Vue 3 global build + Composition API
> - [references/patterns-vue2.md](references/patterns-vue2.md) —— Vue 2 + Options API
>
> 📚 **框架無關的樣板與資源**（響應式簡報引擎、ECharts／Chart.js 視覺化、oklch 配色系統、字體建議、配色×字體搭配表）：
> - [references/patterns-advanced.md](references/patterns-advanced.md)
...
- ✅ 品牌本身就使用 emoji → 跟著品牌
- ✅ **裝置外框狀態列的系統圖示為刻意例外** —— 模擬 iPhone／Android 狀態列的電量／訊號（如 `⚡ 📶`）與時鐘（`9:41`）同屬裝置 mockup 慣例，用以還原真實裝置外觀，不視為「emoji 當 icon 替代品」
...
> 簡報引擎為純 HTML + JS，不依賴框架 —— 見 [references/patterns-advanced.md](references/patterns-advanced.md#響應式簡報引擎)。
...
> 完整範例（ECharts 初始化、tooltip、dark mode 切換、ResizeObserver）見 [references/patterns-advanced.md](references/patterns-advanced.md#資料視覺化樣板)
...
> 各框架的版本鎖定 CDN 見各自的 references 檔（[react](references/patterns-react.md) / [vue3](references/patterns-vue3.md) / [vue2](references/patterns-vue2.md)）—— 不要改動版本。
...
- [ ] 沒有使用 `scrollIntoView`
- [ ] 框架專屬硬規則已遵守（見對應的 references 檔：[React](references/patterns-react.md)／[Vue 3](references/patterns-vue3.md)／[Vue 2](references/patterns-vue2.md)）
...
- [references/patterns-advanced.md](references/patterns-advanced.md) —— 響應式簡報引擎、ECharts／Chart.js 快速上手、oklch 配色系統、字體建議、配色×字體搭配表
