...
> - **Codex**：`ultra`（Sol／Terra 支援的執行模式：max 推理＋自動子代理）＞ `max`（最深單任務推理，本 skill 預設）＞ `xhigh` ＞ `high` ＞ `medium` ＞ `low`
>   - ⚠ **Sol 的原廠預設推理檔是 `low`**，不顯式傳 `--config model_reasoning_effort='"max"'` 就不會是最深推理
>
...
> - Codex：`gpt-5.6-sol` → `gpt-5.6-terra` → `gpt-5.6-luna`（三者皆支援 `max`；`gpt-5.5` / `gpt-5.4` 仍在型錄可用但最深僅到 `xhigh`）
>   - ⚠ **務必顯式帶 `-m`**：CLI 型錄快取若因 schema 不相容載入失敗，預設模型會悄悄退回 `gpt-5.5 + high`（2026-07-20 實測；詳見 dispatch-codex 技能之「常見錯誤與處理」）。顯式指定 slug 可繞過，本技能範例皆已顯式指定
>   - ✅ ChatGPT Plus 在 Codex 可用 gpt-5.6 三階與 `max`（實測確認）；官方對 Plus 的推理檔限制只作用於 ChatGPT 標準對話介面
