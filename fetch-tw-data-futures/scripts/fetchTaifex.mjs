 ...
 * 抓取台指期行情、三大法人期貨未平倉、Put/Call Ratio
 *
 * @module fetchTaifex
 */
...
// ---------------------------------------------------------------------------
async function fetchFuturesData(dateSlash) {
...
// ---------------------------------------------------------------------------
async function fetchInstitutionalData(dateSlash) {
...
// C. Put/Call Ratio
// ---------------------------------------------------------------------------
async function fetchPCRatio(dateSlash) {
...
/**
 * Fetch all TAIFEX data for a given date.
 *
...
export async function fetchTaifex(dateStr) {
    ...
    // If all three failed, throw so the caller knows it's a complete failure
    const hasAnyData = futures || institutional || pcRatio;
    if (!hasAnyData) {
        const err = new Error(`所有資料抓取失敗: ${errors.join('; ')}`);
        err.errors = errors;
        throw err;
    }
