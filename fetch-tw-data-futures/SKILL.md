...
建議使用本技能附帶的 Node.js 腳本進行抓取，穩定性高；依序抓取三類資料（每支間隔約 1 秒以降低 TAIFEX 限流風險），採部分成功機制。
...
腳本會依序抓取三類資料（每支請求間隔約 1 秒以降低 TAIFEX 限流風險），採部分成功機制，任一類失敗不影響其他類：
...
腳本採用**部分成功**機制：三類資料依序抓取（每支間隔約 1 秒），任一類失敗不影響其他類。失敗的類別會記錄在 `errors` 陣列中，對應欄位設為 `null`。

### 5. 三大法人無法解析身份別

**症狀**：`errors` 含「三大法人期貨未平倉：無法解析身份別」。

**原因**：下載到的 CSV 有資料列，但「身份別」欄位用語全部無法對應（外資及陸資／外資／投信／自營商），通常為官方變更欄位用語所致。此時 `institutional` 欄位設為 `null` 並記入 `errors`，避免回傳空物件被誤判為成功而靜默缺資料。
