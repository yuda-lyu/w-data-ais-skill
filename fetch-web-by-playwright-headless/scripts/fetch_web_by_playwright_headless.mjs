#!/usr/bin/env node
// fetch_web_by_playwright_headless.mjs — CLI 包裝
//
// 用法:
//   node fetch_web_by_playwright_headless.mjs <url> [outputPath]
...
if (args.length === 0) {
  process.stderr.write('Usage: node fetch_web_by_playwright_headless.mjs <url> [outputPath]\n');
  process.exit(1);
}
...
(async () => {
  const result = await fetchWebByPlaywrightHeadless(url);
