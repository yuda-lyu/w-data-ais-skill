#!/usr/bin/env node
// fetch_web_by_playwright_head.mjs — CLI 包裝
//
// 用法:
//   node fetch_web_by_playwright_head.mjs <url> [outputPath]
...
if (args.length === 0) {
  process.stderr.write('Usage: node fetch_web_by_playwright_head.mjs <url> [outputPath]\n');
  process.exit(1);
}
...
(async () => {
  const result = await fetchWebByPlaywrightHead(url);
