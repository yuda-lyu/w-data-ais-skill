...
run('fetch-mops',         'fetch-tw-news-mops/scripts/fetch_mops.mjs',                 [raw('mops.json')],               360000); // 最多 6 分鐘（launch retry 60s×N + goto retry + API fetches）
run('fetch-cnyes',        'fetch-tw-news-cnyes/scripts/fetch_cnyes.mjs',               [raw('cnyes.json')],               60000);
run('fetch-statementdog', 'fetch-tw-news-statementdog/scripts/fetch_statementdog.mjs', [raw('statementdog.json')],        60000);
run('fetch-moneydj',      'fetch-tw-news-moneydj/scripts/fetch_moneydj.mjs',           [raw('moneydj.json')],            300000); // 最多 5 分鐘
...
for (let i = 0; i < MAX_INST_LOOKBACK; i++) {
    ...
    const twseOk = run('fetch-twse-t86',
        'fetch-tw-data-institutional/scripts/fetch_twse_t86.mjs',
        ['all', instDate, raw('institutional_twse.json')], 60000);

    if (twseOk) {
        // TWSE 成功 → 確認為交易日，接著抓 TPEX
        run('fetch-tpex-3insti',
            'fetch-tw-data-institutional/scripts/fetch_tpex_3insti.mjs',
...
if (instFetched) {
    ...
    run('fetch-twse-prices',
        'fetch-tw-data-stock/scripts/fetch_twse_stock.mjs',
        ['all', instDate, raw('prices_twse.json')], 60000);
    run('fetch-tpex-prices',
        'fetch-tw-data-stock/scripts/fetch_tpex_stock.mjs',
...
if (instFetched) {
    log(`抓取期貨資料（${instDate}）...`);
    run('fetch-taifex',
        'fetch-tw-data-futures/scripts/fetch_taifex.mjs',
...
if (instFetched) {
    ...
    run('fetch-twse-margin',
        'fetch-tw-data-margin/scripts/fetch_twse_margin.mjs',
        ['all', instDate, raw('margin_twse.json')], 60000);
    run('fetch-tpex-margin',
        'fetch-tw-data-margin/scripts/fetch_tpex_margin.mjs',
