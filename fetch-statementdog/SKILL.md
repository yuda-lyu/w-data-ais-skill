...
3. **解析輸出**：腳本會將結果以 JSON 格式輸出（包在 `JSON_OUTPUT_START` 與 `JSON_OUTPUT_END` 之間），並**一律寫入檔案**（若指定 outputPath 則使用該路徑，否則自動備份至 `statementdog_data.json`）。

```bash
# 範例：無指定路徑，自動備份至 statementdog_data.json
