...
description: 將使用者指定的單一檔案、多個檔案、或資料夾壓縮為 zip 檔，可指定輸出檔名、壓縮等級、密碼（單檔/多檔/資料夾各模式均支援）。底層引擎：無密碼時，單一輸入走 w-zip 的 zipFile/zipFolder，多輸入走 archiver；有密碼時所有模式統一走 archiver-zip-encrypted（zip20 預設，aes256 可選）。壓縮等級預設 1（最快速壓縮）。
---
...
| 條件 | 引擎 |
|------|------|
| **有密碼**（任一模式） | `archiver` + `archiver-zip-encrypted`（隨 w-zip 一起安裝） |
| 無密碼，1 個檔案 | `w-zip` 的 `mZip.zipFile` |
| 無密碼，1 個資料夾 | `w-zip` 的 `mZip.zipFolder` |
| 無密碼，2+ 個輸入（或混合） | `archiver`（純 zip） |

> **為何多輸入不走 w-zip**：w-zip 僅提供 `zipFile`（單檔）與 `zipFolder`（單資料夾），無法直接接受多輸入。若先將多檔案搬入暫存資料夾再呼叫 `zipFolder`，zip 內會多出該暫存資料夾名作為頂層目錄。因此多輸入改用 archiver 直接組合，於 zip 根層級放置各檔案，與一般使用者預期一致。
>
> **為何有密碼一律走 archiver**：w-zip 對單檔/單資料夾的密碼加密本身就是用 `archiver-zip-encrypted`，且 hardcode `encryptionMethod: 'zip20'`。本技能直接使用 `archiver-zip-encrypted` 是同一套底層，但能讓 `aes256` 等選項對所有模式可用，行為更一致。

## 安裝指引

> **[執行AI須先依照技能內說明安裝指定依賴之套件]**

所需 npm 套件：`w-zip`（其依賴鏈會自動帶入 `archiver`、`archiver-zip-encrypted`、`unzipper` 等）

執行前驗證：
```bash
node -e "require('w-zip'); require('archiver'); require('archiver-zip-encrypted'); console.log('w-zip + archiver + archiver-zip-encrypted OK')"
```
...
| `--encryption zip20\|aes256` | — | `zip20` | 加密方法（`--password` 有指定時生效，見下節） |
...
依全庫慣例（僅 `--json` 模式適用）：
- `status: "success"` — 壓縮完成
- `status: "error"` — 輸入不存在、輸出寫檔失敗、level 不合法、encryption 不合法等
...
- `archiver-zip-encrypted` 格式於進程內首次使用時延遲註冊，重複註冊以 try/catch 守住
...
6. **檔名重複**：多檔案模式若兩個輸入有相同 basename（例：`./a/x.txt` 與 `./b/x.txt`），zip 內後加入者會覆蓋前者；建議避免此情境或改用單一資料夾模式
