...
整合 10 個新聞來源（RSS + AI News Aggregator + Hacker News），取得資料後統一填入 `from` 欄位，並過濾為僅保留今日與昨日（UTC+8）的消息，依時間降冪排序輸出。
...
| Hacker News | Firebase API | https://news.ycombinator.com/newest | https://hacker-news.firebaseio.com/v0/newstories.json |
...
| 唐国梁Tommy | YouTube RSS | https://www.youtube.com/@TGLTommy | https://www.youtube.com/feeds/videos.xml?channel_id=UCVezb06mOvkXDwB8ibaOlYQ |
...
```text
10 個來源並行取得（Promise.allSettled，每支單獨加 45s race timeout 上限，避免單支跑滿重試拖慢整批；整批耗時 = 最慢那支）
→ RSS 來源呼叫 fetch-rss/scripts/fetchRSS.mjs
→ AI News Aggregator 呼叫 fetch-ai-news-aggregator/scripts/fetchAiNewsAggregator.mjs
→ Hacker News 呼叫 fetch-hacker-news/scripts/fetchHackerNews.mjs
→ 各自填入指定 from 欄位，提取 title 欄位
→ 彙整為單一陣列
→ 套用網域黑名單（AI 選文前移除無法摘要的來源，預設：gelonghui.com）
→ 過濾僅保留今日與昨日（UTC+8）
→ 依時間降冪排序
→ 輸出 JSON 陣列
```
...
部分網站文章內容極短（多為一行快訊），無法供後續 AI 選文／摘要使用，會在彙整後、AI 選文前依 URL 網域剔除。

比對方式：取 URL 的 hostname，若等於黑名單項目或為其子網域（例如 `www.gelonghui.com` 屬於 `gelonghui.com`），即剔除。

黑名單定義於 [fetch-news-ai/scripts/fetchNewsAi.mjs](scripts/fetchNewsAi.mjs) 的 `BLOCKED_URL_DOMAINS`：

| 網域 | 對應來源 | 排除原因 |
|------|----------|----------|
| `gelonghui.com` | 格隆汇 | 文章多為一行快訊，內容過短無法摘要 |
...
| 錯誤 | 原因 | 解法 |
|------|------|------|
| `[fetch-news-ai] xxx 失敗` | 單一來源取得失敗 | 該來源會被跳過，其餘來源不受影響（內建重試機制：5xx / 429 / 網路錯誤自動重試最多 5 次，含初始請求最多執行 6 次；RSS 來源另含 404 重試） |
