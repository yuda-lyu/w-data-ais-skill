...
> 須從**技能庫根目錄**（具有 `node_modules` 所在位置）執行。本技能透過相對路徑載入 `fetch-rss` 與 `fetch-ai-news-aggregator`，三者須為同層兄弟資料夾。
...
| 錯誤 | 原因 | 解法 |
|------|------|------|
| `[fetch-news-ai] xxx 失敗` | 單一來源取得失敗 | 該來源會被跳過，其餘來源不受影響（內建重試機制：5xx / 404 / 網路錯誤自動重試最多 5 次） |
