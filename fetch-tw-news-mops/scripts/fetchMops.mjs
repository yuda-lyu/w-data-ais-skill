 ...
 *
 * 匯出：
 *   fetchMops() => Promise<{ results: Array, hasError: boolean }>
 */
...
/**
 * 抓取 MOPS 重大公告資料
 * @returns {Promise<{ results: Array, hasError: boolean }>}
 * @throws {Error} 瀏覽器啟動失敗時拋出錯誤
 */
export async function fetchMops() {
    console.log('啟動瀏覽器...');
    let browser;
    for (let attempt = 1; attempt <= MAX_RETRIES + 1; attempt++) {
        try {
            ...
            const attemptsLeft = MAX_RETRIES + 1 - attempt;
            if (attemptsLeft <= 0) {
                throw new Error(`瀏覽器啟動失敗（已重試 ${MAX_RETRIES} 次）: ${e.message}`);
            }
            const delay = Math.min(BASE_DELAY_MS * attempt, MAX_DELAY_MS);
