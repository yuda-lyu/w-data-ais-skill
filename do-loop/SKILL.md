...
```jsonc
{
  ...
  "phase": "EXECUTE",             // "EXECUTE" | "AUDIT" | "FIX"
...
| 事件 | 更新欄位 |
|------|---------|
| PLAN 完成 | 建立 state.json，`phase: "EXECUTE"`, 建立 `tasks` 陣列 |
...
IF 無待修任務（無可繼續 FIX 者）且 至少一任務 failed → status: "halted"（部分通過、部分需人工介入）
...
| ❓ 需求不明 | PLAN 階段模糊需求 | 暫停提問，回答後繼續 | 尚未建立（PLAN 完成才寫入） |
