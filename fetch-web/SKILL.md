---
name: fetch-web
description: 抓取任意網頁的文章內容並提取純文字。支援四種方法自動階梯升級：curl+Readability（繞過 TLS 指紋）→ Playwright 無頭（SPA 渲染）→ Playwright 有頭（反自動化偵測）→ Camofox 反偵測瀏覽器（Cloudflare Turnstile 等進階驗證）。適用於 AI Agent 自動擷取新聞全文、文章摘要等場景。
---
...
4. **Camofox 反偵測瀏覽器** — 使用 Camoufox（修改版 Firefox）繞過 Cloudflare Turnstile 等進階反自動化偵測，透過 accessibility snapshot 取得結構化頁面資料（如微信公眾號）
...
> ⚠️ Playwright 為必裝依賴。本技能依網站防禦強度自動階梯升級（curl → Playwright 無頭 → Playwright 有頭 → Camofox），長期執行必定會遇到需要 Playwright 的場景，未安裝將導致整個模組無法載入。
...
> Camofox 為修改版 Firefox（Camoufox），可繞過 Cloudflare Turnstile 等進階反自動化偵測。方法④ 透過 accessibility snapshot 取得結構化頁面資料，適用於方法①②③ 皆無法通過驗證的網站。
...
│
└─ 方法④ Camofox 反偵測瀏覽器
     Camoufox（修改版 Firefox）
     → 啟動 Camofox server
     → 建立 tab 瀏覽目標 URL
     → 取得 accessibility snapshot
     → 轉換為 HTML 供 Readability 解析
     → 繞過 Cloudflare Turnstile 等進階反自動化偵測
```
...
| ④ Camofox 反偵測瀏覽器 | 1-4+（含 Cloudflare Turnstile） | 慢（15-30s） | Cloudflare Turnstile 等進階驗證（微信公眾號） |
...
- 所有方法（curl / Playwright 無頭 / Playwright 有頭 / Camofox）皆內建自動重試（最多重試 5 次，含初始請求最多執行 6 次，線性遞增退避 3s→6s→...→15s）；curl 針對 5xx/429 及網路錯誤重試，Playwright 針對導航逾時、瀏覽器 crash 等例外重試
...
```bash
node fetch-web/scripts/fetch_web.mjs <url> [outputPath] [--method=auto|curl|playwright|playwright-headed|camofox]
```
...
# 強制使用 Camofox 反偵測瀏覽器（適合 Cloudflare Turnstile 等進階驗證）
node fetch-web/scripts/fetch_web.mjs "https://mp.weixin.qq.com/s/xxx" ./result.json --method=camofox
```
