...
> **欄位說明**：TPEX 端點回傳的 JSON `fields` 為「裸欄名」——`買進股數/賣出股數/買賣超股數` 各重複 7 次（外資不含自營 / 外資自營 / 外資合計 / 投信 / 自營自行 / 自營避險 / 自營合計），群組標籤不在 JSON 內。本技能以**固定位置**還原為具語意欄名，並**對齊 TWSE T86 命名**（`外陸資.../投信.../自營商...`），使上市/上櫃資料可共用同一套 key；末欄保留 `三大法人買賣超股數合計`。每檔共 24 欄，下列僅示意常用欄位。若 TPEX 改版導致欄序/欄數變動，腳本會直接報錯（不會靜默輸出錯誤資料）。
