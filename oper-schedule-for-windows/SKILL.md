...
### 視窗顯示：會不會彈出 cmd 黑視窗，由「執行 session」決定

任務跑在哪個 session，決定有沒有視窗——這是 LogonType 的**直接副作用**，不需任何額外設定：
...
> **機制**：session 0 是 Windows 給服務用的非互動 session，**沒有可顯示視窗的桌面**，所以視窗根本無從出現；子程序繼承同一 session，也一律無視窗。換言之，若你已為「登出也跑＋需網路」選了 Password／SYSTEM，**無視窗是免費附帶的**。

**常見誤解**：`<Hidden>true</Hidden>` **不是**用來隱藏視窗的——它只把任務從「工作排程器資料庫」清單藏起來不顯示，與視窗無關。

**若非得用 InteractiveToken 又要無視窗**：動作直接指向 `.exe`（而非 `cmd.exe /c …`）可少一層 cmd，但 console 程式自身在互動 session 仍會有視窗；要徹底隱藏，讓排程動作改呼叫 `wscript.exe` 跑一支隱藏啟動的 `.vbs`：

```vbs
' run-hidden.vbs — 第 2 引數 0=隱藏視窗，False=不等它結束
CreateObject("WScript.Shell").Run "node C:\path\app.mjs", 0, False
```

排程動作設成 `Command=wscript.exe`、`Arguments=C:\path\run-hidden.vbs`。
